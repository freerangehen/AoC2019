...
def output_(arg1, arg1_mode, ram):
    value = (1-arg1_mode)*ram[arg1] + arg1_mode*arg1
    print(value)
    return ram


def input_(arg1, arg1_mode, ram):
...
if __name__ == "__main__":
    ...
    assert (dispatch[op](*args_and_modes, [1, 2, 3, 4, 33]) == [1, 2, 3, 4, 99])
    idx += idx_inc[op]
    assert idx == 4
    ...
    while idx < len(code):
        ...
        if op != '99':
            ...
            break
        args_and_modes = list(chain.from_iterable(zip(args_, args_mode)))
        code = dispatch[op](*args_and_modes, code)
        idx += idx_inc[op]
