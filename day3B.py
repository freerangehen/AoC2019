...
def run_wire(direction, start_x, start_y, distance, grid, nearest_cross, 
             shortest_dist, old_grid, dist_so_far):
    """start, end are inclusive
    """
    steps_moved = 1
    while steps_moved <= distance:
        if direction == 'L':
            current_x = start_x - steps_moved
            current_y = start_y
        if direction == 'R':
            current_x = start_x + steps_moved
            current_y = start_y
        if direction == 'U':
            current_x = start_x
            current_y = start_y + steps_moved
        if direction == 'D':
            current_x = start_x
            current_y = start_y - steps_moved
        if ((current_x, current_y) in old_grid) and ((current_x, current_y) != (0, 0)):
            if (current_x, current_y) not in grid:
                total_dist = dist_so_far + steps_moved + old_grid[(current_x, current_y)]
            else:
                total_dist = grid[(current_x, current_y)] + old_grid[(current_x, current_y)]

            if total_dist < shortest_dist:
                shortest_dist = total_dist
                nearest_cross = (current_x, current_y)

        if (current_x, current_y) not in grid:
            grid.update({(current_x, current_y): dist_so_far + steps_moved})

        steps_moved += 1

    return current_x, current_y, grid, nearest_cross, shortest_dist, dist_so_far
...
if __name__ == "__main__":
    ...
    for each_section in line1:
        ...
        nearest_cross, shortest_dist,\
        dist_so_far = run_wire(direction, current_x, current_y, distance, 
                                                grid1, nearest_cross, 
                                                shortest_dist, empty_grid,
                                                dist_so_far=dist_so_far)
